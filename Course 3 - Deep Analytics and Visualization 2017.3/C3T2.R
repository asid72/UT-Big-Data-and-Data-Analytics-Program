...
hhpwrDT <- hhpwrDT %>%
  mutate(year = year(DateTime), month = month(DateTime), day = day(DateTime), week=week(DateTime), minute=minute(DateTime),
         weekday = weekdays.POSIXt(DateTime), hour=hour(DateTime))
...
### Second Plot
## Subset the 9th day of January 2008 - 10 Minute frequency
houseDayWeek <- filter(hhpwrDT, year == 2009 & month == 1 & week == 1 &(hour==0 | hour==6|hour==12|hour==18|hour==24))


## Plot sub-meter 1, 2 and 3 with title, legend and labels - 10 Minute frequency
plot_ly(houseDayWeek, x = ~houseDayWeek$DateTime, y = ~houseDayWeek$Sub_metering_1, name = 'Kitchen', type = 'scatter', mode = 'lines') %>%
  add_trace(y = ~houseDayWeek$Sub_metering_2, name = 'Laundry Room', mode = 'lines') %>%
  add_trace(y = ~houseDayWeek$Sub_metering_3, name = 'Water Heater & AC', mode = 'lines') %>%
  layout(title = "Power Consumption January 1-8th, 2008",
         xaxis = list(title = "Time"),
         yaxis = list (title = "Power (watt-hours)"))

## Subset the 9th day of January 2008 - 10 Minute frequency
houseDayWeek2 <- filter(hhpwrDT, year == 2009 & month == 2 & week == 6 &(hour==0 | hour==6|hour==12|hour==18|hour==24))


## Plot sub-meter 1, 2 and 3 with title, legend and labels - 10 Minute frequency
plot_ly(houseDayWeek2, x = ~houseDayWeek2$DateTime, y = ~houseDayWeek2$Sub_metering_1, name = 'Kitchen', type = 'scatter', mode = 'lines') %>%
  add_trace(y = ~houseDayWeek2$Sub_metering_2, name = 'Laundry Room', mode = 'lines') %>%
  add_trace(y = ~houseDayWeek2$Sub_metering_3, name = 'Water Heater & AC', mode = 'lines') %>%
  layout(title = "Power Consumption Feb 5-12th, 2009",
         xaxis = list(title = "Time"),
         yaxis = list (title = "Power (watt-hours)"))

##Step 2:
hhpwrDT$weekday
#hhpwrDT <- hhpwrDT$weekday
##as.integer(hhpwrDT$weekday)
## Subset to one observation per week on Mondays at 8:00pm for 2007, 2008 and 2009
house070809weekly <- filter(hhpwrDT, weekday == "Monday" & hour == 20 & minute == 1)
summary(hhpwrDT)
summary(house070809weekly)
str(house070809weekly)
count(house070809weekly)
...
#Plot 2, Step 2:
houseTuesdayweekly <- filter(hhpwrDT, weekday == "Tuesday" & hour == 20 & minute == 1)
tsSM3_Tuesdayweekly <- ts(houseTuesdayweekly$Sub_metering_1, frequency=52, start=c(2007,1))

autoplot(tsSM3_Tuesdayweekly)

#plot 2, step 3:
houseWednesdayweekly <- filter(hhpwrDT, weekday == "Wednesday" & hour == 20 & minute == 1)
tsSM2_Wednesdayweekly <- ts(houseWednesdayweekly$Sub_metering_2, frequency=52, start=c(2007,1))

autoplot(tsSM2_Wednesdayweekly)
...
fitSM1 <- tslm(tsSM3_Tuesdayweekly ~ trend + season) 
summary(fitSM1)
...
# Step 3, plot 3
#tsSM2_Wednesdayweekly
fitSM2 <- tslm(tsSM2_Wednesdayweekly ~ trend + season) 
summary(fitSM2)
...
#Compare
Compare <- resamples(list(lm2=fitSM2, lm1=fitSM1, lm3=fitSM3))
...
componentsTuesdaym1weekly <- decompose(tsSM3_Tuesdayweekly)
## Plot decomposed sub-meter 1 
plot(componentsTuesdaym1weekly)
## Check summary statistics for decomposed sub-meter 1 
summary(componentsTuesdaym1weekly)

#Step 4, plot 3:
## Decompose Sub-meter 1 into trend, seasonal and remainder
componentsWednesdaym2weekly <- decompose(tsSM2_Wednesdayweekly)
## Plot decomposed sub-meter 1 
plot(componentsWednesdaym2weekly)
## Check summary statistics for decomposed sub-meter 2 
summary(componentsWednesdaym2weekly)
...
## Seasonal adjusting sub-meter 3 by subtracting the seasonal component & plot
tsSM1_Adjusted <- tsSM3_Tuesdayweekly - componentsTuesdaym1weekly$seasonal
...
plot(tsSM1_HWfor, ylim = c(0, 20), ylab= "Watt-Hours", xlab="Time - Sub-meter 3")

## Forecast HoltWinters with diminished confidence levels
tsSM1_HWforC <- forecast(tsSM1_HW, h=25, level=c(10,25))
## Plot only the forecasted area
plot(tsSM1_HWforC, ylim = c(0, 20), ylab= "Watt-Hours", xlab="Time - Sub-meter 1", start(2010))
...
## Seasonal adjusting sub-meter 3 by subtracting the seasonal component & plot
tsSM2_Adjusted <- tsSM2_Wednesdayweekly - componentsWednesdaym2weekly$seasonal
...
plot(tsSM2_HWfor, ylim = c(0, 20), ylab= "Watt-Hours", xlab="Time - Sub-meter 2")

## Forecast HoltWinters with diminished confidence levels
tsSM2_HWforC <- forecast(tsSM2_HW, h=25, level=c(10,25))
## Plot only the forecasted area
plot(tsSM2_HWforC, ylim = c(0, 20), ylab= "Watt-Hours", xlab="Time - Sub-meter 2", start(2010))
